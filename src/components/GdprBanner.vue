...
onMounted(() => {
  const consent = localStorage.getItem('mobilynx_consent')
  if (!consent) setTimeout(() => { visible.value = true }, 1200)
})

const accept = () => {
  localStorage.setItem('mobilynx_consent', JSON.stringify({ all: true, timestamp: Date.now() }))
  visible.value = false
}

const decline = () => {
  localStorage.setItem('mobilynx_consent', JSON.stringify({ all: false, timestamp: Date.now() }))
  visible.value = false
}

const savePrefs = () => {
  const prefs = categories.value.reduce((acc, c) => ({ ...acc, [c.id]: c.enabled }), {})
  localStorage.setItem('mobilynx_consent', JSON.stringify({ prefs, timestamp: Date.now() }))
  visible.value = false
}
</script>
...
.gdpr-banner {
  ...
  max-width: 980px;
  margin: 0 auto;
  background: rgba(6, 36, 26, 0.97);
  border: 1px solid rgba(20,184,106,0.3);
  border-radius: 20px;
  backdrop-filter: blur(24px);
  z-index: 9999;
  box-shadow: 0 24px 80px rgba(0,0,0,0.5), 0 0 0 1px rgba(20,184,106,0.1);
  overflow: hidden;
}
...
.gdpr-btn-accept:hover { transform: translateY(-1px); box-shadow: 0 4px 20px rgba(20,184,106,0.4); }
...
.gdpr-btn-settings:hover { border-color: var(--primary); color: var(--purple-light); }
